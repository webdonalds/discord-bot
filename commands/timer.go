...
func (c *TimerCommand) Execute(args []string, _ *discordgo.MessageCreate) (responses.ResponseMessage, background.Watcher, error) {
	if len(args) != 2 {
		return responses.NewTextMessage(timerHelpMsg), nil, nil
	}

	duration, err := time.ParseDuration(args[0])
	if err != nil {
		return responses.NewTextMessage("시간 패턴을 파싱할 수 없습니다."), nil, nil
	}
	...
	return responses.NewTextMessage(msg), watcher, nil
}
